...
void RequestQueue::insertRequest(std::size_t filesize, std::string fileName, std::shared_ptr<TCPconnection_server> new_connection) { 
	dwld_request new_request;
	new_request.connection = new_connection;
	new_request.fileName = fileName;
	new_request.fileSize = filesize;
	if (insertElement(new_request) == false) { throw RequestInsertionException(); }
}