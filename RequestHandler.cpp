...
bool RequestHandler::send_request(const std::string server, char* file_path) {
	...
	std::cout << file_request.file_name_ << " DESTINAZIONE: " << file_request.destination_address_ << std::endl <<
		"DIMENSIONE: " << file_request.file_size_ << std::endl;

	FILETIME write_time;
	if (!file_handler.get_file_time(nullptr, nullptr, &write_time)) {
		std::cout << "CANNOT READ FILE TIME" << std::endl;
		return false;
	}

	std::cout << WindowsFileHandler::format_file_time(write_time) << std::endl;

	requests_.push_back(file_request);

	upload_manager_->upload_file(std::make_shared<FileRequest>(file_request), std::move(file_handler));
